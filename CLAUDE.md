...
| 2026-09-13 | Orbite : aperçu tronqué (texte simple, 70 caractères, infobulle pour le reste) dans la liste des cartes d'un paquet, plus recentrage vertical de la ligne (`docs/etude-flashcards.md` §21). Jules : « [l'affichage] rend très moche [avec] des longues réponses... il faut recentrer certains éléments et surtout ne pas afficher toute la réponse... juste le début et peut-être l'afficher entier si on clique sur modifier ou même en tooltip » | Le HTML n'est jamais tronqué directement (couperait des balises ouvertes) : `lib/htmlPreview.ts` (bibliothèque pure, testée, même discipline que `lib/richText.ts`) le réduit d'abord en texte simple, puis coupe sur la dernière frontière de mot. `CardPreview.tsx` affiche ce texte dans un `<span title="…">` — l'infobulle native du navigateur, la deuxième option proposée par Jules, sans rien construire. `RichText.tsx` (affichage intégral et mis en forme) reste réservé à l'écran de révision, où lire toute la réponse est le but. `align-items: flex-start` sur `.flashcards-card-row` n'avait de raison d'être que pour un verso multi-lignes (§17-§18) ; l'aperçu étant maintenant une seule ligne, la règle est retirée — la ligne retrouve le centrage vertical du socle, réponse directe au « recentrer ». `overflow: hidden; white-space: nowrap; text-overflow: ellipsis` en renfort CSS pur (`.flashcards-card-front`/`-back`, `.flashcards-row-name`) : si l'estimation en caractères ne colle pas exactement à la largeur réelle sur un écran étroit, l'ellipse referme proprement. Le repli mobile qui empilait recto/verso disparaît avec la même logique. `554/554` tests unitaires → `564/564` (+10), `466/466` local → `468/468` (+2), `478/478` en mode comptes → `480/480` (+2) |
| 2026-09-13 | Orbite : la palette de l'éditeur passe de 4 à 8 mises en forme — italique, barré, code en ligne, liste numérotée (`docs/etude-flashcards.md` §20). Jules, après §19 : « pourquoi y'a que 4 options de markdown ? » puis « rajoute quelques éléments souvent utilisés » | Les quatre premières (§17-§18) n'étaient pas une limite technique, seulement ce qui avait été demandé mot pour mot — répondu en toute franchise avant d'agir. Choisies pour un recto/verso court, pas un document long ; laissés de côté (titres, citation, lien, règle horizontale), toujours aussi faciles à rallumer si besoin. Coût quasi nul : les quatre étaient déjà dans `StarterKit`, simplement désactivées en §19 — retirer leur `: false` dans `CardEditor.tsx` et ajouter quatre entrées au tableau `BUTTONS` de `EditorToolbar.tsx` (déjà piloté par une liste plutôt que du JSX répété, pensé pour ça). `.flashcards-format-toolbar` gagne `flex-wrap: wrap` : huit boutons ne tiennent pas forcément sur une ligne en largeur mobile. `554/554` tests unitaires (inchangé), `464/464` local → `466/466` (+2), `476/476` en mode comptes → `478/478` (+2) |
| 2026-09-13 | Orbite : §17 (syntaxe maison) et §18 (boutons sur du texte brut) remplacés par un éditeur de texte riche, **Tiptap** — première dépendance UI externe d'Atlas (`docs/etude-flashcards.md` §19). Jules, après avoir essayé les boutons de §18 : « il faut sélectionner un texte pour que ça marche [liste], et si je fais entrée ça continue pas la liste... je préférerai une bibliothèque » | react-quill écarté (manipule le DOM lui-même via `findDOMNode`, retiré de React 18+, pas de fork officiel React 19) ; Tiptap déclare React 19 dans ses `peerDependencies`, licence MIT. Coût assumé et documenté : +124 ko gzippés sur le bundle JS (ProseMirror). `front`/`back` deviennent du HTML (`editor.getHTML()`), toujours de simples chaînes en base — aucune migration. Schéma de l'éditeur volontairement restreint (`StarterKit.configure` désactive titres/citation/code/lien/liste numérotée/italique/barré) : un schéma plus large que les boutons qui l'exposent serait un piège, un raccourci clavier ferait apparaître ce qu'aucun bouton ne pourrait retirer. `RichText.tsx` passe le tout premier `dangerouslySetInnerHTML` d'Atlas — jugé sûr : ce HTML ne sort que du schéma restreint de l'éditeur, jamais d'une saisie brute copiée telle quelle. Les deux défauts signalés disparaissent structurellement : `useEditorState` (l'API recommandée par Tiptap) synchronise l'état actif de chaque bouton avec le curseur, pas avec une sélection ; Entrée continue une liste nativement (`ListKeymap`, dans `StarterKit`) puisqu'une liste y est un vrai nœud du document. **Vrai bug trouvé en écrivant la suite e2e** (pas qu'un souci de test) : avec deux éditeurs Tiptap sur la même page (recto et verso), une fois l'un des deux focus au moins une fois, `editor.chain().focus()` ne redonnait plus le focus de façon fiable après un clic sur un bouton de l'autre — le focus restait sur le `<button>`, et l'espace tapé juste après le réactivait nativement, défaisant la mise en forme qu'on venait de poser. Corrigé par le motif standard des barres d'outils de texte riche : `onMouseDown={(e) => e.preventDefault()}` sur chaque bouton, pour que le focus ne quitte jamais l'éditeur. `554/554` tests unitaires (`richText.test.ts`/`textEditing.test.ts` supprimés avec les fichiers qu'ils couvraient, jamais committés), `465/465` local → `464/464`, `477/477` en mode comptes → `476/476` |
| 2026-09-13 | Orbite : boutons d'aide à la rédaction (Gras, Souligner, Surligner, Liste) sur les champs recto/verso (`docs/etude-flashcards.md` §18), et le gras rejoint la mise en forme légère de §17. Demande de Jules : « on peut afficher du texte markdown mais est-ce qu'il y a les petits outils d'aide à la rédaction qui permettent de mettre en gras, faire une liste, souligner… » | Le gras (`**texte**`) manquait à §17 (liste/souligné/surligné seulement) — ajouté puisqu'une quatrième marque était de toute façon nécessaire pour que les boutons couvrent ce qui était demandé ; c'est la seule des quatre qui coïncide avec CommonMark. `isListLine` exportée de `lib/richText.ts` plutôt que redéfinie, pour que le bouton Liste et l'affichage s'accordent toujours sur ce qu'est une puce. Deux fonctions pures et testées dans `lib/textEditing.ts` (même discipline que `lib/boxes.ts`) : `wrapSelection` entoure la sélection d'une marque et **la retire si elle y est déjà** (bascule, comme n'importe quel éditeur enrichi), `toggleListPrefix` fait la même bascule ligne à ligne. `FormatToolbar.tsx` (un par `<textarea>`) lit la sélection sur l'élément DOM au clic — elle survit au transfert de focus vers le bouton — puis restaure le focus et la position du curseur après coup, ce qu'un changement de `value` piloté par état ne fait pas tout seul. `566/566` tests unitaires → `580/580` (+14), `462/462` local → `465/465` (+3), `474/474` en mode comptes → `477/477` (+3) |
| 2026-09-12 | Orbite : mise en forme légère du recto/verso — listes, souligné, surligné (`docs/etude-flashcards.md` §17), post-V1 (§3 excluait toute mise en forme). Demande de Jules : « je veux pouvoir faire du markdown : je veux pouvoir faire des listes, souligner, surligner » | Pas du Markdown/CommonMark complet, trois règles propres à Orbite : une ligne commençant par `- `/`* ` pour une puce, `__texte__` pour souligner, `==texte==` pour surligner — `__texte__` voudrait dire gras en CommonMark, mais rien n'était demandé au-delà de ces trois usages. Aucune migration, `front`/`back` restent de simples chaînes : la syntaxe n'existe que dans l'interprétation qu'on en fait à l'affichage. Bibliothèque pure d'abord (`lib/richText.ts`, testée avant tout écran, même discipline que `lib/boxes.ts`), puis un composant `RichText.tsx` qui construit des éléments React directement plutôt que du HTML injecté (`dangerouslySetInnerHTML` n'existe nulle part dans Atlas). Utilisé aux trois endroits qui affichaient du texte brut (liste des cartes, liste agrégée par boîte, écran de révision) ; `CardEditor` gagne un aperçu qui suit la frappe — un `<textarea>` ne peut pas afficher lui-même une puce ou un surlignage — et un rappel de la syntaxe en `.field-hint` (classe déjà partagée par le socle). `554/554` tests unitaires → `566/566` (+12), `460/460` local → `462/462` (+2), `472/472` en mode comptes → `474/474` (+2) |
| 2026-09-07 | Hub : une description sous le nom de chaque module, dans `ModulePicker` (socle). Jules : « appart Zénith, c'est dur de comprendre ce que le module fait » — question ouverte sur un rebrand complet des quatre noms, plusieurs pistes proposées (noms fonctionnels purs, thème céleste enrichi, deux nouveaux thèmes transparents, correctif ciblé sur Astra seul). Décision : pas de renommage pour l'instant, juste plus de contexte | Nouveau champ `description: string` sur `AtlasModule` (`core/lib/module.ts`), obligatoire — un module sans description serait un module illisible dans le hub, pas une omission tolérable. Rendu sous `label` dans `ModulePicker.tsx`, seul fichier du socle touché en dehors du contrat lui-même. Un `.hub-picker-card` a déjà trois enfants (emoji, label) empilés avec un `gap` uniforme ; plutôt que de restructurer en sous-groupes, `.hub-picker-description` reprend un `margin-top` négatif pour se rapprocher du nom sans toucher à l'espacement emoji→nom, plus généreux. Aucune vérification n'existait jusqu'ici pour `ModulePicker` lui-même (chaque suite de module ne fait qu'y passer en chemin) : deux ajoutées dans `e2e/core.mjs`, le seul endroit socle qui a le droit de le faire. `554/554` tests unitaires (inchangé, contenu statique sans logique pure à tester à part), `458/458` local → `460/460` (+2), `470/470` en mode comptes → `472/472` (+2) |
| 2026-09-07 | Astra : deux nouvelles sections sur l'onglet Évolution — Entrées et Différentiel (`docs/etude-astra.md`, section ajoutée). Demande de Jules : « il me faudrait aussi l'évolution de l'épargne : combien j'ai eu de rentrés par mois, puis faire le différentiel entre les deux : de combien je suis en négatif ou positif chaque mois pour comparer » — « épargne » pris au sens large de Jules (l'argent qui rentre), pas la nature de catégorie `epargne` au sens strict, qui a déjà sa propre courbe sur l'onglet Épargne | `computeIncomeTrend` (pendant exact de `computeSpendingTrend` côté entrées) et `computeNetTrend` (rejoue **exactement** la formule déjà affichée sous « Solde » sur l'onglet Aperçu, mois par mois — jamais une nouvelle définition inventée à côté), dans `lib/spendingTrend.ts`, qui gagne au passage un `monthRange` partagé par les trois fonctions. Nouveau type `NetPoint` (peut être négatif, à la différence de `TrendPoint`, documenté comme une magnitude) et nouveau composant `NetTrendChart` — pas une variante de `SpendingTrendChart`, dont les barres supposent un socle fixe en bas du cadre, incompatible avec les deux signes : celles du différentiel partent d'une ligne zéro au milieu, vers le haut (excédent, vert) ou vers le bas (déficit), mêmes teintes que `.budget-month-stat-net`. Contrairement au retrait du signe décidé la veille pour les dépenses, le différentiel **garde** son signe partout : ici, le signe est l'information demandée. Trois sections empilées, toujours visibles ensemble plutôt que derrière un onglet ou une bascule — comparer, c'est les garder sous les yeux en même temps. `545/545` tests unitaires → `554/554` (+9), `448/448` local → `458/458` (+10 : les trois sections visibles ensemble, entrées filtrées par catégorie, absence de sélecteur sur le différentiel, mois déficitaire resté négatif au tableau et sur le graphe avec sa classe CSS, plus l'onglet Évolution ajouté à la vérification de rendu mobile), `460/460` en mode comptes → `470/470` (+10). Un échec préexistant sans rapport (`Aucune animation permanente sur mobile hors la flamme`, suite Zénith) rencontré en vérifiant, confirmé flaky indépendamment de ce chantier (reproduit sur `main` sans aucune modification Astra) — non corrigé ici, hors périmètre |
| 2026-09-06 | Astra : deux retouches à l'onglet Évolution, retour immédiat de Jules — plus de signe « - » sur des montants qui ne peuvent être que des dépenses, et le montant affiché directement sur chaque barre plutôt qu'au survol seulement | `formatSpent` bascule de `formatCents` (toujours signé) vers `centsToInputValue`, déjà utilisé sans signe pour la stat « Entré » de l'onglet Aperçu. Un `<text>` par mois, toujours affiché même à zéro, arrondi à l'euro pour tenir au-dessus d'une barre étroite — le survol garde le montant exact au centime dans l'infobulle. `545/545` tests unitaires (inchangé), `445/445` local → `448/448` (+3), `457/457` en mode comptes → `460/460` (+3) |
| 2026-09-06 | Astra : onglet Évolution (courbe de dépense multi-mois) et comparatif mois-à-mois (`docs/etude-astra.md` §12), après une recherche de marché demandée par Jules avant de coder quoi que ce soit — « le but de faire un budget, c'est... pouvoir faire évoluer ma façon de dépenser et voir cette évolution au fil du temps ». YNAB/Copilot/Monarch/Linxo étudiés ; une étude comportementale (Irrational Labs) montre qu'un plafond avec voyant rouge n'a aucun effet net sur la dépense — d'où le choix explicite d'un comparatif en teinte discrète plutôt qu'une alerte, et le report à plus tard d'un plafond par catégorie (proposé, refusé par Jules pour l'instant). Détection des abonnements proposée aussi, mise de côté par Jules (« je dois réfléchir à plusieurs choses sur ça ») | `computeSpendingTrend` rejoue `computeMonthlyBreakdown` mois par mois plutôt que de dupliquer ses règles (rollup, exclusions, remboursements) ; `SpendingTrendChart` reprend le langage visuel de `PPChart` (Zénith) sans en réutiliser le code. `monthDelta`/`deltaMap`/`formatMonthDelta` masquent tout comparatif tant qu'aucune écriture n'existe avant le mois affiché (sinon un « nouveau » trompeur sur chaque ligne du tout premier mois). Bug mobile trouvé en vérifiant le 5ᵉ onglet, même famille que celui du 24/08/2026 mais cause différente : `.main` (socle) se centre avec `margin: 0 auto`, ce qui désactive l'étirement flex une fois que `.layout` passe en colonne sur mobile — `.budget-main` reprenait alors la largeur de son contenu (436px) plutôt que celle du viewport (390px). Corrigé avec un `width: 100%` sur `.budget-main` sous 760px, sans toucher au socle. `526/526` tests unitaires → `545/545` (+19), `434/434` local → `445/445` (+11), `446/446` en mode comptes → `457/457` (+11) |
| 2026-09-06 | Astra : le bouton flottant d'une enveloppe affichait encore son ancien texte (`docs/etude-astra.md` §11). Rapporté par Jules : « on voit "nouvelle..." qqchose écrit sur le bouton » sur l'onglet Épargne | `EnvelopesScreen.tsx` n'avait jamais reçu la migration vers le bouton icône-seule faite pour les catégories et les écritures (§8) : il gardait son texte d'origine (« + Nouvelle enveloppe ») tout en héritant la classe `.budget-add` (rond, 46 px), qui débordait au lieu de disparaître derrière `aria-label`. Jamais couvert par les vérifications : la suite ne crée qu'une seule enveloppe, toujours via le bouton de l'écran vide, jamais via ce bouton flottant. Corrigé à l'identique du motif déjà en place ailleurs, vérification ajoutée pour ne plus perdre ce cas de vue. `526/526` tests unitaires (inchangé), `432/432` local → `434/434` (+2), `444/444` en mode comptes → `446/446` (+2) |
| 2026-09-06 | Astra : raccourci clavier « N » pour une nouvelle écriture (`docs/etude-astra.md` §10), demandé par Jules. Posé dans `EntriesView.tsx`, qui possède déjà l'état d'édition et le bouton flottant — actif seulement sur l'onglet Aperçu, là où « nouvelle écriture » a un sens. Premier raccourci lettre seule d'Atlas : jusqu'ici, seul `Escape` fermait déjà une fenêtre partout, sans jamais vérifier où était le focus | Trois garde-fous ajoutés faute de précédent à copier : ignoré pendant la frappe (focus sur un champ éditable), ignoré si un éditeur est déjà ouvert, ignoré avec un modificateur (Cmd/Ctrl+N reste la nouvelle fenêtre du navigateur). Annoncé dans le titre du bouton flottant (« Nouvelle écriture (N) ») plutôt que caché. `526/526` tests unitaires (inchangé, comportement UI sans nouvelle logique pure à tester à part), `427/427` local → `432/432` (+5 : ouverture au clavier, non-interférence en tapant un « n », fermeture, écriture réellement enregistrée), `439/439` en mode comptes → `444/444` (+5) |
| 2026-09-06 | Astra : sous-catégories, un seul niveau (`docs/etude-astra.md` §9) — « Restaurants & bars » → « Restaurants », « Bar », ou « Loisirs » → « Cartes One Piece ». Demande de Jules, avec la question ouverte « plusieurs catégories, ou catégories puis sous-catégories ? ». Étude écrite d'abord, tranchée avec lui : sous-catégories (pas multi-tag, qui ferait mentir le total du mois — une écriture reste dans une seule catégorie), un seul niveau de profondeur, `kind` hérité du parent. Pour le camembert, trois maquettes comparées en artifact (rollup avec petite liste de détail / drill-down / second camembert permanent) : Jules a choisi la petite liste | `parentId` nullable sur `BudgetCategory` (migration `2026-09-06-budget-subcategories.sql`, `on delete set null` — jamais `cascade` : supprimer un parent **promeut** ses sous-catégories, ne les efface jamais, même philosophie que « à classer » un niveau plus haut). Profondeur à un niveau et héritage de `kind` vérifiés en TypeScript (`lib/categoryHierarchy.ts`, testé), pas en CHECK SQL. Le camembert ne fait jamais qu'une part par catégorie parente (`rollupKey`) ; `SubcategoryDetail` affiche le détail juste au-dessus de la liste filtrée, avec une part « Non précisé » (couleur du parent) pour ce qui est posé directement dessus. Piège trouvé en vérifiant : une catégorie enregistrée avant ce chantier n'a pas de `parentId` du tout (`undefined`, pas `null`) — normalisé à la lecture dans `LocalBudget.read()`, sans quoi elle aurait disparu de tous les groupes ; repéré par des vérifications post-V1 existantes, cassées net avant ce correctif. `507/507` tests unitaires → `526/526` (+19 : `categoryHierarchy.test.ts` 6, hiérarchie dans `localBudget.test.ts` 6, rollup et détail dans `monthlyBreakdown.test.ts` 7), `413/413` local → `427/427` (+14), `425/425` en mode comptes → `439/439` (+14) |
| 2026-09-06 | Zénith : une bandelette des sept derniers jours sous la flamme du bandeau d'accueil (`docs/etude-quotidien.md` §2, annotée) — flamme allumée les jours faits, gel ❄ les jours couverts par un gel, flamme éteinte les jours vraiment manqués. Demande de Jules : « il faudrait afficher la streak des derniers jours en plus, avec une petit flamme si j'ai eu le jour, une gel si j'ai utilisé un gel, et une flamme eteinte si j'ai rien fait et que j'avais plus de gel » — exactement le manque identifié dès l'étude d'août (« on a les gels, il leur manque le calendrier qui les montre ») | `computeStreak` ne renvoyait qu'un total agrégé, jamais le détail jour par jour nécessaire à la bandelette. Refactorisé en `walkStreak`, une marche interne unique dont `computeStreak` ne garde que le résultat final et le nouveau `recentStreakDays` (`lib/streak.ts`) le détail complet — jamais deux calculs qui pourraient diverger, seule garantie qui vaille pour un streak. Un jour antérieur au tout premier jour d'activité (ou aujourd'hui, tant que la journée n'est pas finie) est rendu `'pending'`, neutre, plutôt que `'missed'` — sans quoi un compte flambant neuf afficherait sept flammes éteintes avant même d'avoir commencé. `490/490` tests unitaires → `507/507` (+17 : `recentStreakDays` 6, plus la couverture existante de `computeStreak` rejouée telle quelle pour vérifier que le refactor ne change aucun résultat), `408/408` local → `413/413` (+5 : sept jours affichés, aujourd'hui « fait », jours d'avant neutres, un jour gelé affiché comme tel, le run consécutif autour intact), `420/420` en mode comptes → `425/425` (+5) |
| 2026-09-06 | Zénith : le vrai bug du cumul multi-actions (deux entrées du dessous) enfin cerné et corrigé (`docs/etude-paliers-comptables.md` §14, deux sections ajoutées) — le masquage-si-zéro de l'entrée précédente traitait le symptôme, pas la cause. Rapporté par Jules après un nouveau test : « pour mon objectif anglais qui se mesure en jours j'ai toujours ... 0 jours cette semaine ... alors que j'ai coché l'action duolinguo ». Le palier était en réalité **« Jours » (`compte`)**, pas `cumul` — et `weeklyGoalAmount` sommait `contribution` pour tous les paliers sans distinction, alors qu'un palier `compte`/`serie` ne quantifie **jamais** ses actions par construction (compter des jours ne demande aucune unité) : la somme valait donc toujours zéro, structurellement, masquer un zéro ne suffisait pas | `weeklyGoalAmount` regarde maintenant `ladderKind(goal.tiers).kind` : `compte`/`serie` comptent des **jours distincts** (même dédoublonnage que `distinctDays` côté palier), `cumul`/`performance` somment `contribution` comme avant, `mesure` renvoie `null` (sommer des relevés n'a pas de sens, `MeasureChart` fait déjà ce travail). Correctif complémentaire trouvé en creusant : une action ajoutée *après* la création d'un objectif `cumul`/`performance` (via `ActionEditor`) ne portait jamais de quantité, contrairement aux deux actions génériques posées à la création — nouvelle fonction `inheritedActionAmount` (`lib/quantities.ts`, même calcul que `starterActions`), branchée dans `ZenithScreen.tsx`. `490/490` tests unitaires → `501/501` (+11 : `inheritedActionAmount` 5, `weeklyGoalAmount` compte/série/mesure 6), `404/404` local → `408/408` (+4 : héritage de l'unité à l'ajout, cas « Jours » affiché sans quantification manuelle), `416/416` en mode comptes → `420/420` (+4) |
| 2026-09-06 | Zénith : le cumul multi-actions (entrée du dessous) corrigé et rendu optionnel par objectif (`docs/etude-paliers-comptables.md` §14, section ajoutée). Rapporté par Jules le jour même : « Apprendre l'anglais » (cumul, une action « Duolingo » cochée sans quantité) affichait un graphe présent mais entièrement à zéro — le check-in existe, mais rien à sommer sans action quantifiée en face. « ca me met un graph vide avec 0 jours... mais j'ai pas forcément envie de voir ça sur cette metrique » | `weeklyGoalAmount` ne renvoyait `null` que si **aucune** réalisation n'alimentait l'objectif — un total réel mais nul (des réalisations existent, juste rien à quantifier) passait au travers, d'où le graphe plat trompeur. `GoalCard` distingue maintenant l'automatique (masqué si `total === 0`) du choix explicite : nouveau `Goal.trackAmount: boolean | null` (`null` = automatique), `updateGoal(id, { trackAmount })` sur le contrat + les deux implémentations, migration `2026-09-06-goals-track-amount.sql`. Le choix explicite l'emporte toujours : un « × » masque un cumul réel qu'on ne veut pas voir (le marathon), un lien discret « Afficher le cumul par semaine » réaffiche un cumul nul qu'on veut quand même voir (l'anglais, à 0 en toutes lettres — honnête plutôt que masqué en douce). `490/490` tests unitaires (inchangé, la logique de bascule vit dans le composant, pas dans une fonction pure à tester à part), `398/398` local → `404/404` (+6 : masquer/rouvrir un cumul réel, cumul à zéro auto-masqué, lien de réouverture, affichage forcé à zéro), `410/410` en mode comptes → `416/416` (+6) |
| 2026-09-06 | Zénith : un cumul multi-actions à l'échelle de l'objectif, pas seulement du palier (`docs/etude-paliers-comptables.md` §14) — « combien de km cette semaine, en tout, depuis le début », en additionnant plusieurs actions différentes ET les gestes ponctuels quantifiés. Demande de Jules sur un objectif marathon (palier `performance`, qui ne somme jamais) : « je veux savoir combien j'ai couru de km dans toutes mes actions de l'objectif de la semaine, il peut y avoir plusieurs actions différentes, et même les actions ponctuelles ». `goalAmountCheckins`/`contribution` (exportée) dans `lib/counters.ts`, `weeklyGoalAmount` dans `lib/progress.ts` (même repli hebdomadaire à trous comblés que `weeklyPP`), affiché par `GoalAmountChart` sur la carte de l'objectif dès que `ladderKind` résout une unité non mixte. Seule vraie extension de modèle : `addOneOff` gagne un `value` optionnel (contrat + les deux implémentations + l'outbox/`sync.ts`, qui portait déjà un champ `value` générique), et la barre de saisie d'un geste ponctuel affiche un champ de quantité quand l'objectif a une unité connue | Une métrique **de l'objectif**, pas d'un palier : contrairement à un palier `cumul` (une seule échelle, jamais de geste ponctuel) ou `performance` (la meilleure séance, jamais une somme), rien n'existait pour répondre à « le total, tout confondu ». `goalAmountCheckins` reprend la règle de `feedsByDefault` (une mesure comme la VMA n'alimente jamais un cumul) mais, à la différence de `feedingCheckins`, inclut explicitement les gestes ponctuels — c'est tout l'objet de la demande. Le total ne remplace la barre de progression d'aucun palier existant : les deux peuvent légitimement diverger (le geste ponctuel compte ici, pas dans le palier), et c'est voulu. Gating sur `!ladder.mixed` : une échelle mixte n'a pas d'unité de confiance à afficher pour un cumul. `490/490` tests unitaires → (+17 : `counters.test.ts` `goalAmountCheckins`/`contribution`, `progress.test.ts` `weeklyGoalAmount`), `393/393` local → `398/398` (+5 : champ de quantité affiché, geste ponctuel quantifié, cumul affiché sur la carte, total et semaine en cours corrects), `405/405` en mode comptes → `410/410` (+5) |
| 2026-08-31 | Astra : le « + » du bouton flottant recentré (`docs/etude-astra.md` §8, addendum) — signalé pas centré verticalement dans le rond | Mesuré avant de corriger : la boîte flex qui contient l'icône est parfaitement centrée (0 px d'écart), mais le glyphe « + » de la police ne l'est pas dans sa propre boîte de texte — un défaut de métriques de police, pas de mise en page. Corrigé en dessinant le « + » en CSS pur (`.budget-add-icon`, deux barres via `::before`/`::after`, positionnées indépendamment) plutôt qu'un décalage approximatif qui n'aurait tenu que par hasard sur une police et un navigateur donnés. `477/477` tests unitaires, `393/393` local, `405/405` en mode comptes — tous inchangés (CSS et markup d'icône uniquement) |
| 2026-08-31 | Astra : la barre pleine largeur remplacée par un bouton flottant (FAB) — rond, 46px, icône « + » seule, coin bas-droit (`docs/etude-astra.md` §8, section ajoutée). Retour de Jules quelques minutes après le commit précédent : « le bouton est beaucoup trop gros et visible » | Motif étudié avant d'implémenter : le FAB de Material Design (Gmail, Keep, Trello mobile) est le standard précisément pour ce geste — toujours atteignable sans dominer l'écran. Le nom accessible (« Nouvelle écriture »/« Nouvelle catégorie ») vit désormais dans `aria-label`/`title` plutôt que dans le texte visible, même motif que les boutons icône-seule de la barre du haut d'Orbite. `477/477` tests unitaires (inchangé, CSS et markup de bouton uniquement), `393/393` local et `405/405` en mode comptes (inchangés en nombre — les vérifications existantes ont été adaptées au nouveau nom accessible et à la nouvelle taille, pas remplacées) |
| 2026-08-31 | Astra : le bouton sticky retravaillé en barre pleine largeur (`docs/etude-astra.md` §8, section ajoutée), et le rendu mobile vérifié pour la première fois — jamais fait jusqu'ici pour ce module, malgré des correctifs mobiles ponctuels fin août. Retour de Jules : « le système est cool, mais l'UI n'est pas top » | La pastille alignée à gauche manquait de présence ; `align-self: stretch` + texte centré en fait une vraie barre d'action, alignée avec la largeur de la liste. En vérifiant le mobile, un vrai bug trouvé : les lignes de catégorie (`.budget-row` nue dans `BudgetScreen.tsx`) n'avaient jamais reçu le repli sous 520px déjà appliqué aux écritures et aux enveloppes — un nom de catégorie un peu long débordait avec les deux boutons d'action. Corrigé avec sa propre classe (`.budget-category-row`) et le même motif. `477/477` tests unitaires (inchangé, CSS et classes uniquement), `383/383` local → `393/393` (+10 : barre pleine largeur, quatre onglets et éditeur vérifiés sans débordement sur 390px), `395/395` en mode comptes → `405/405` (+10) |
