...
/**
 * Un bloc de vérifications isolé du reste du fichier.
 *
 * Sans ceci, un élément introuvable coûtait BEAUCOUP plus cher qu'un échec :
 * `locator.evaluate()` attend trente secondes puis lève, le script mourait sur
 * place, et tout ce qui suivait n'était pas vérifié du tout — sans que la
 * ligne finale « X/Y vérifications passées » ne s'affiche pour le signaler.
 * C'est ce qui est arrivé tous les dimanches pendant des semaines : le
 * parcours avec comptes passait silencieusement à la trappe.
 *
 * Désormais un plantage coûte UNE vérification en échec, nommée, et la suite
 * du fichier continue.
 */
async function section(nom, fn) {
  try {
    ...
    check(`Le bloc « ${nom} » s'est interrompu`, false, String(error?.message ?? error).split('\n')[0]);
  }
}
...
// Plus de liste de rangs à l'ajout : le rang vient de la place. Le formulaire
// annonce celui à venir au lieu de faire choisir un rang qu'il n'honorait plus.
check(
...
// 3 bis. Insérer un palier au milieu — le seul chemin offert est « ajouter à
// la fin, puis remonter ». Il produisait une échelle qui redescend, parce que
// le rang restait collé au palier au lieu de rester sur son barreau.
await section('insérer un palier au milieu', async () => {
    const rangs = async () =>
      (await page.locator('.ladder .tier .rank-badge').allTextContents()).join(' | ');
    const avant = await rangs();
    const ligne = page.locator(".ladder .tier", { hasText: 'Courir 42 km sous les 4 h' });
    await ligne.getByRole('button', { name: /^Monter/ }).click();
    await page.waitForTimeout(400);
    check(
      'Remonter un palier ne change pas la suite des rangs de l’échelle',
      (await rangs()) === avant,
      `${avant}  →  ${await rangs()}`,
    );
    const ordre = await page.locator('.ladder .tier .tier-title').allTextContents();
    check(
      'Mais le palier a bien changé de place',
      ordre.indexOf('Courir 42 km sous les 4 h') === ordre.length - 2,
      ordre.join(' | '),
    );
    // Et on le remet où il était, pour ne pas perturber la suite du fichier.
    await ligne.getByRole('button', { name: /^Descendre/ }).click();
    await page.waitForTimeout(400);
});
...
// Le halo de l'anneau terminé était tranché net au bord du viewport SVG : le
// cercle s'arrête à 7 px du bord pour un flou qui en demande 10.
check(
  'Le halo de l’anneau n’est pas rogné par la boîte du SVG',
  (await page.locator('.ring-wrap svg').evaluate((el) => getComputedStyle(el).overflow)) ===
    'visible',
  await page.locator('.ring-wrap svg').evaluate((el) => getComputedStyle(el).overflow),
);
check(
...
await section('animations coupées si l’utilisateur les refuse', async () => {
...
await section('barre du haut et navigation', async () => {
...
await section('service worker et rappels', async () => {
...
if (process.env.AUTH_BASE) {
  await section('parcours avec comptes', async () => {
...
await section('revenir sur les jours précédents', async () => {
...
await section('paliers comptables', async () => {
...
await section('quantités, relevés et mesures', async () => {
    const fresh = await browser.newContext({ viewport: { width: 1150, height: 1000 } });
...
await section('mise en page de la création d’objectif', async () => {
...
await section('rattrapage d’un palier déjà atteint', async () => {
...
await section('réglages et sauvegarde', async () => {
...
await section('grille des jours', async () => {
...
await section('filtrer la grille par action', async () => {
    const fresh = await browser.newContext({ viewport: { width: 1180, height: 950 } });
...
await section('accompagnement par utilisateur', async () => {
...
await section('nature d’un objectif et insertion', async () => {
    const fresh = await browser.newContext({ viewport: { width: 1100, height: 950 } });
    const cp = await fresh.newPage();
    cp.on('pageerror', (e) => errors.push(e.message));
    await cp.goto(BASE);
